...
export class WindowManager {
  ...
  async injectText(cellId: string, text: string): Promise<boolean> {
    ...
    if (isKimiUrl(url) || isTongyiUrl(url) || isChatglmUrl(url)) {
      const result = await this.injectNativeClickSiteText(cellId, text, url);
      if (!result) {
        this.showCellNotice(cellId, 'inject-failed');
      }
      return result;
  ...
  private async injectNativeClickSiteText(cellId: string, text: string, url: string): Promise<boolean> {
    if (this.isDestroyed()) {
      return false;
    }
    ...
    try {
      ...
      const prepared = isKimiUrl(url)
        ? await this.executeCellScript(cellId, buildKimiPrepareScript(text))
        : await this.prepareNativeTextInput(view.webContents, cellId, url, text);
      ...
      if (isChatglmUrl(url)) {
        await this.executeCellScript(cellId, buildChatglmSubmitScript(text));
        let accepted = await this.executeCellScript(cellId, buildNativeClickAcceptedScript(url));
        if (accepted === true) {
          return true;
        }
      }
      ...
      if (isChatglmUrl(url)) {
        const focused = await this.executeCellScript(cellId, buildNativeInputFocusScript(url));
        ...
        accepted = await this.executeCellScript(cellId, buildNativeClickAcceptedScript(url));
      }
  ...
  private async prepareNativeTextInput(
    ...
    return this.executeCellScript(cellId, buildNativeClickButtonScript(url, text));
  }
...
function isKimiUrl(rawUrl: string): boolean {
  try {
    const hostname = new URL(normalizeUrl(rawUrl)).hostname.toLowerCase().replace(/^www\./, '');
    return hostname === 'kimi.com' || hostname === 'kimi.moonshot.cn';
  } catch {
    return false;
  }
}

function isTongyiUrl(rawUrl: string): boolean {
  try {
    const hostname = new URL(normalizeUrl(rawUrl)).hostname.toLowerCase().replace(/^www\./, '');
    return hostname === 'qianwen.com' || hostname === 'chat.qwen.ai' || hostname === 'tongyi.aliyun.com';
  } catch {
    return false;
  }
}

function isChatglmUrl(rawUrl: string): boolean {
  try {
    const hostname = new URL(normalizeUrl(rawUrl)).hostname.toLowerCase().replace(/^www\./, '');
    return hostname === 'chatglm.cn';
  } catch {
    return false;
  }
}

function buildNativeClickAcceptedScript(rawUrl: string): string {
  if (isKimiUrl(rawUrl)) {
    return KIMI_SEND_ACCEPTED_SCRIPT;
  }

  if (isTongyiUrl(rawUrl)) {
    return TONGYI_SEND_ACCEPTED_SCRIPT;
  }

  if (isChatglmUrl(rawUrl)) {
    return CHATGLM_SEND_ACCEPTED_SCRIPT;
  }

  return 'false';
}

function buildNativeInputFocusScript(rawUrl: string): string {
  if (isTongyiUrl(rawUrl)) {
    return buildCoordinateInputFocusScript([
...
function buildNativeClickButtonScript(rawUrl: string, text: string): string {
  if (isTongyiUrl(rawUrl)) {
    return buildCoordinateClickButtonScript({
      text,
      inputSelectors: [
        'textarea:not([disabled])',
        '[contenteditable="true"][role="textbox"]',
  ...
  if (isChatglmUrl(rawUrl)) {
    return buildChatglmClickButtonScript({
      text,
      inputSelectors: [
        'textarea:not([disabled])',
        '[contenteditable="true"][role="textbox"]',
  ...
  return `
    (async () => {
      ...
      for (let attempt = 0; attempt < 30; attempt += 1) {
        ...
        if (button && isVisible(button) && currentText && currentText === expectedText) {
  ...
  return `
    (async () => {
      ...
      for (let attempt = 0; attempt < 30; attempt += 1) {
        ...
        if (button && currentText && currentText === expectedText) {
...
function buildChatglmSubmitScript(text: string): string {
  return `
    (async () => {
      ...
      const input = findInput();
      if (!input || textOf(input) !== expectedText) return false;
...
const KIMI_SEND_ACCEPTED_SCRIPT = `
  (async () => {
    const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
    const getInputText = () => {
...
const TONGYI_SEND_ACCEPTED_SCRIPT = buildTextOrGeneratingAcceptedScript([
  'textarea:not([disabled])',
  '[contenteditable="true"][role="textbox"]',
  '[role="textbox"]:not([aria-disabled="true"])',
  '[contenteditable="true"]',
...
const CHATGLM_SEND_ACCEPTED_SCRIPT = buildTextOrGeneratingAcceptedScript([
  'textarea:not([disabled])',
  '[contenteditable="true"][role="textbox"]',
  '[role="textbox"]:not([aria-disabled="true"])',
  '[contenteditable="true"]',

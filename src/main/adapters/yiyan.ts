...
export const yiyanAdapter: SiteAdapter = {
  urlPattern: /https:\/\/(?:yiyan|chat)\.baidu\.com/i,
  injectScript: (text: string) => buildYiyanInjectScript(text),
  readyCheckScript: buildYiyanReadyScript(),
};

function buildYiyanReadyScript(): string {
  return `
...
function buildYiyanInjectScript(text: string): string {
  return `
    (async () => {

...
export const copilotAdapter: SiteAdapter = {
  ...
  injectScript: (text: string) => buildCopilotInjectScript(text),
  readyCheckScript: buildCopilotReadyScript(),
};

function buildCopilotReadyScript(): string {
  return `
...
function buildCopilotInjectScript(text: string): string {
  return `
    (async () => {
      const targetText = ${JSON.stringify(text)};
      const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
      const input = [...document.querySelectorAll('textarea, [contenteditable="true"][role="textbox"], [contenteditable="true"]')].at(-1);

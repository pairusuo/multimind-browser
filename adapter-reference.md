...
>
> 当前「生成总结文档」简易流程只向用户选定的总结者 AI 发送总结指令，
> 要求 AI 输出可复制保存的 Markdown 文档，不自动读取总结结果、不弹预览、
> 不落盘。因此本章节的读取适配器当前主要服务转发/交叉验证链路；后续若
> 重新接入自动预览或落盘保存，需要先重新评审捕获策略，不能依赖固定超时。
...
|-----|------------|-------------|---------|
| Claude (claude.ai) | 模式一，识别真实停止控件（`button[aria-label="Stop response"]`） | 最新回答取 `[class*="standard-markdown"]`，兼容 `font-claude-message` / `message-content`；完整对话首版按 `data-testid="user-message"` 与 assistant markdown/message 容器提取，过滤按钮、工具栏、输入框等 UI 元素，并按可见 turn 位置排序 | **最新回答与完成判断已验证通过；完整对话提取首版已实现，未实站验证**。验证条件：账号 dirkchou，窗口保持前台且未锁屏。发送按钮现场确认为 `button[aria-label="Send message"]`，点击后 3 秒内出现用户消息、`Claude is responding` 和 `Stop response`；生成中 `isResponseComplete()` 返回 `false`，约 6.8 秒后停止控件消失并返回 `true`；`extractLatestResponse()` 提取到正文 476 字，未混入界面文字 |
| ChatGPT (chatgpt.com) | 模式一，识别真实 `stop` 控件（`button[data-testid="stop-button"]`） | 最新回答取 `[data-message-author-role="assistant"] .markdown`；完整对话首版按 `[data-message-author-role="user/assistant"]` 提取，过滤按钮、工具栏、输入框等 UI 元素，并按 conversation turn 顺序排序 | **最新回答与完成判断已验证通过；完整对话提取首版已实现，待实站验证**。修正记录：最初实现误把"已停止思考"按钮当成正在生成中的信号，导致 `isResponseComplete()` 一直返回 `false`，修正为只识别真实 stop 控件后才正确。后续需要验证用户在 ChatGPT 网页内手动追问后，`extractConversation()` 是否能同时捕获用户问题和 assistant 回复 |

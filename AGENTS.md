...
- "可能未完整记录用户在网页内的手动追问"这类不确定性说明——这是
  说给操作 MultiMind 的用户听的，目标 AI 不需要也不应该看到这句话，
  它对 AI 完成评价任务没有任何帮助，只会造成困惑
- 任何关于"应用内部如何获取这段上下文"的元信息

### 修正：不确定性提示移出 prompt，改为界面提示

之前实现里那句"提示：以下内容来自 MultiMind 已记录的讨论；如果
...
这个格子的对话是通过 MultiMind（底部输入或转发注入）发生的，应用
...
降级为"仅最新回答"。**只有当某一段对话完全是用户绕开 MultiMind、
...
2. **转发交互 UI**（✅ 已完成并验证：基础转发链路、格子时间线架构、
   ...
   **2b. 格子时间线与上下文合并**（已升级为正式架构，不是补丁）：
   ...
   - 站点尚未实现 `extractConversation` 不再直接导致转发降级——只要
     这段对话是经过 MultiMind（底部输入或转发注入）发生的，应用
     自己的时间线记录就是完整的

3. **文档生成**（当前任务）：实现"用户指定一个 AI 汇总"的触发流程
   和 prompt 拼接逻辑。

   **材料范围限定**：
   - 参与本轮讨论的每个格子的**完整时间线**（`CellTimeline`，不是
     简单的"首轮回答"——如果某个格子经历过多轮对话或接收过转发，
     时间线本身已经包含完整过程，直接复用，不要重新简化成"只取
     首轮"）
   - `forwardRecords` 里记录的、用户实际触发过的转发动作（谁转发给
     谁、转发时的上下文、目标的回复）——这部分用于让总结者了解
     "用户主动关注的交叉验证点"，不要默认拉入用户没有触发过的转发
     组合
   - 不要重新发明一套材料收集逻辑，第2步已经验证过的 `getCellFullContext`
     和 `forwardRecords` 直接复用即可

   **总结者选择**：用户从当前布局里参与讨论的格子中选择一个作为
   总结者，触发方式是界面上的"生成总结文档"按钮（之前设计文档里
   提到的交互入口）

   **Prompt 结构**：开篇说明总结者的任务（提炼价值、标注共识/分歧、
   不编造信息、不确定的标注"待核查"）；材料区按"原始问题 → 各格子
   完整时间线 → 实际发生的转发及其回复"组织；输出结构固定为：标题、
   摘要、主要共识、关键分歧与修正、最终结论、待核查事项、可执行
   建议七段式

   **语言规则**：复用已实现的 `detectContentLanguage` 判断这次讨论
   内容使用的语言，总结文档的任务说明文案跟随对话内容语言，不跟随
   界面语言（和转发功能的语言规则保持一致，不要重新设计一套）

   **长度管理**：多个格子的完整时间线加上转发记录，组装后的素材
   很可能超长，复用已有的 `MAX_CONVERSATION_CHARS` 裁切机制和
   `conversation-truncated` 提示，不要重新设计一套长度控制逻辑
